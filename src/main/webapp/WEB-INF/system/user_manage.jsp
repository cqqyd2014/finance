				...
				.datagrid(
						{
...

									{
										...
										formatter : function(value, rec) {
											var btn = '<a class="effective" onclick="effective(\''
													...
													+ '\')" id="effective_'
													+ rec.user_id
													+ '" href="javascript:void(0)">启用</a><a class="user_detail" onclick="user_detail(\''
													+ rec.user_id
													...
													+ '\')" id="del_'
													+ rec.user_id
													+ '" href="javascript:void(0)">停用</a><a class="change_password" onclick="change_password_init(\''
													+ rec.user_id
													+ '\',\''
													+ rec.user_name
													+ '\')" id="change_password_'
							...
							onLoadSuccess : function(data) {
								...
								$('#users_table').datagrid('fixRowHeight');

								var rowsData = $("#users_table").datagrid(
										"getRows");
...
<script>
	function add_user() {
		...
		if (value == '') {
			...
			$.getJSON("../system/add_user.action", $('#new_user_form')
...
<div class="easyui-layout" fit="true" style="background: red;">
	...
	<div id="tools_panel" data-options="region:'south',border:true"
		title='新增用户' style="height: 100px; padding: 5px;">
		<!------------------ 在这里填写你的搜索条件（FORM） -------------------->
		...
		<div>
			<form id="new_user_form">
				<table>
					<tr>
						<td>登录名</td>
						<td><input id='user_login' name='user_login'
					...
					<tr>
						...
						<td>联系电话</td>
						<td><input id='tell' name='tell' class="easyui-textbox"
	...
	<jsp:include page="user_manage_dialog/change_password.jsp" flush="true" />
